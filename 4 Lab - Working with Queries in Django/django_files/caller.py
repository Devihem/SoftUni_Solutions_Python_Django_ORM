...
def find_authors_nationalities():
    authors_filtered_list = Author.objects.exclude(nationality=None)
    return '\n'.join(f"{aut.first_name} {aut.last_name} is {aut.nationality}"
                     for aut in authors_filtered_list)
...
def delete_review_by_id(review_id):
    review_for_delete = Review.objects.filter(id=review_id)
    for rev in review_for_delete:
        log_info = f"Review by {rev.reviewer_name} was deleted"
        review_for_delete.delete()
        return log_info


# print(delete_review_by_id(14))
# print(delete_review_by_id(17))
# print(delete_review_by_id(18))


# 05. Filter Authors by Nationalities
def filter_authors_by_nationalities(author_nationality):
    filtered_authors = Author.objects.filter(nationality=author_nationality).order_by('first_name', 'last_name')

    return_log = []
    for auth in filtered_authors:
        auth_info = f"{auth.biography}"
        if auth_info == 'None':
            auth_info = f"{auth.first_name} {auth.last_name}"

        return_log.append(auth_info)
    return '\n'.join(return_log)
...
def filter_authors_by_birth_year(from_y, to_y):
    filtered_authors = Author.objects.filter(birth_date__year__range=(from_y, to_y)).order_by('-birth_date')

    return_log = []
    for auth in filtered_authors:
        return_log.append(
            f"{auth.birth_date}: {auth.first_name} {auth.last_name}"
        )
    return '\n'.join(return_log)
