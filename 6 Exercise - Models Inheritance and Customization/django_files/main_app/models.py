...
class StudentIDField(models.PositiveIntegerField):
    ...
    def get_prep_value(self, any_data):
        return int(any_data)
...
class BaseReservation(models.Model):
    ...
    def calculate_total_cost(self):
        # return f"{(self.reservation_period() * self.room.price_per_night):.2f}"
    ...
    # Transferred -------------------
    def is_not_available(self, end_date):
        overlap_reservations = self.__class__.objects.filter(
            room=self.room,
    ...
    def clean(self):
        ...
        elif self.is_not_available(self.end_date):
...
class SpecialReservation(BaseReservation):
    ...
    def extend_reservation(self, days: int):
        ...
        if self.is_not_available(new_end_date):
